...
def test_sub_punctuation(
    input_node: nodes.LiteralNode, expected_output: nodes.Node
) -> None:
...
def test_exec_condition(
    input_node: nodes.ConditionNode, input_ctx: Context, expected_node: nodes.Node
) -> None:
...
def test_grammar_node_generate():
    grammar = nodes.Grammar(
        {
            "some_entity": nodes.EntityNode(
                "some_entity", [nodes.LiteralNode("a"), nodes.PlaceholderNode("a")]
            )
        },
        {},
    )
...
@pytest.mark.parametrize(
    ...
    [
        (
            nodes.ConditionNode(
                (nodes.LiteralNode("a"), nodes.LiteralNode("b")), nodes.LiteralNode("c")
            ),
            nodes.ConditionNode(
                (nodes.LiteralNode("a"), nodes.LiteralNode("b")), nodes.LiteralNode("c")
            ),
        ...
        (
            nodes.ConditionNode(
                (nodes.LiteralNode("b"), nodes.LiteralNode("b")), nodes.LiteralNode("c")
            ),
            nodes.ConditionNode(
                (nodes.LiteralNode("a"), nodes.LiteralNode("b")), nodes.LiteralNode("c")
            ),
        ...
        (
            nodes.ConditionNode(
                (nodes.LiteralNode("a"), nodes.LiteralNode("x")), nodes.LiteralNode("c")
            ),
            nodes.ConditionNode(
                (nodes.LiteralNode("a"), nodes.LiteralNode("b")), nodes.LiteralNode("c")
            ),
        ...
        (
            nodes.ConditionNode(
                (nodes.LiteralNode("a"), nodes.LiteralNode("b")), nodes.LiteralNode("a")
            ),
            nodes.ConditionNode(
                (nodes.LiteralNode("a"), nodes.LiteralNode("b")), nodes.LiteralNode("c")
            ),
...
def test_condition_node_eq(
    node_a: nodes.Node, node_b: nodes.Node, should_eq: bool
) -> None:
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (
            nodes.ConditionNode(
                (nodes.LiteralNode("a"), nodes.LiteralNode("a")), nodes.LiteralNode("a")
            ),
            {},
            "a",
            None,
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (
            nodes.ParameterNode("hello", nodes.PlaceholderNode("a")),
            {},
            "",
            RuntimeError,
        ),
...
def test_parameter_node_generate(
    node: nodes.ParameterNode,
    ctx: dict,
    expected_output: str,
    want_err: Optional[Exception],
) -> None:
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (
            nodes.MacroNode("asdf", [nodes.ParameterNode("a")], []),
            nodes.MacroNode("asdf", [nodes.ParameterNode("a")], []),
            True,
        ),
        (
            nodes.MacroNode("asdf", [nodes.ParameterNode("a")], []),
            nodes.MacroNode("asdf", [nodes.ParameterNode("b")], []),
            False,
        ),
        (
            nodes.MacroNode("asdf", [], [nodes.LiteralNode("a")]),
            nodes.MacroNode("asdf", [], [nodes.LiteralNode("a")]),
            True,
        ),
        (
            nodes.MacroNode("asdf", [], [nodes.LiteralNode("a")]),
            nodes.MacroNode("asdf", [], [nodes.LiteralNode("b")]),
            False,
        ),
    ],
)
...
def test_macro_reference_node_eq(
    node_a: nodes.Node, node_b: nodes.Node, should_eq: bool
) -> None:
    assert should_eq == (node_a == node_b)
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (
            nodes.EntityNode("a", [], macro=nodes.MacroReferenceNode("a")),
            nodes.EntityNode("a", [], nodes.MacroReferenceNode("a")),
            True,
        ),
    ],
)
def test_entity_node_eq(
    node_a: nodes.Node, node_b: nodes.Node, should_eq: bool
) -> None:
    assert should_eq == (node_a == node_b)
...
@pytest.mark.parametrize(
    ...
    [
        (
            nodes.EntityNode(
                "a", [nodes.LiteralNode("some"), nodes.LiteralNode("BODY")]
            ),
            {},
            "someBODY",
            None,
        ...
        (
            nodes.EntityNode(
                "a", [nodes.LiteralNode("some"), None, nodes.LiteralNode("BODY")]
            ),
            {},
            "someBODY",
            None,
        ...
        (
            nodes.EntityNode(
                "a",
                [
                    nodes.LiteralNode("some"),
                    nodes.LiteralNode(""),
                    nodes.LiteralNode("BODY"),
                ],
            ),
            {},
            "someBODY",
        ...
        (
            nodes.EntityNode("a", [nodes.PlaceholderNode("a")]),
            {"a": "someBODY"},
            " someBODY",
            None,
        ),
...
def test_entity_node_generate(
    node: nodes.EntityNode,
    ctx: dict,
    expected_output: str,
    want_err: Optional[Exception],
) -> None:
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (
            nodes.AnyNode([nodes.LiteralNode("a"), nodes.PlaceholderNode("b")]),
            {"b": ["c", "x", "z"]},
            {"a", " c", " x", " z"},
        ),
    ],
...
@pytest.mark.parametrize(
    ...
    [
        (
            nodes.OptionalNode(nodes.LiteralNode("a")),
            nodes.OptionalNode(nodes.LiteralNode("a")),
            True,
        ),
        (
            nodes.OptionalNode(nodes.LiteralNode("a")),
            nodes.OptionalNode(nodes.LiteralNode("b")),
            False,
        ),
    ],
)
def test_optional_node_eq(
    node_a: nodes.Node, node_b: nodes.Node, should_eq: bool
) -> None:
    assert should_eq == (node_a == node_b)
...
def test_optional_node_generate():
    ...
    for i in range(1000):
        with mock.patch.object(
            node.expression, "generate", return_value=node.expression.generate()
        ) as mock_gen:
            val = node.generate()
            if val == "":
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (
            nodes.ListNode([nodes.LiteralNode("a")]),
            nodes.ListNode([nodes.LiteralNode("b")]),
            False,
        ),
        (
            nodes.ListNode([nodes.LiteralNode("a")]),
...
@pytest.mark.parametrize(
    ...
    [
        (
            nodes.ListNode([nodes.LiteralNode("some"), nodes.LiteralNode("BODY")]),
            {},
            "someBODY",
            None,
        ),
        (
            nodes.ListNode(
                [nodes.LiteralNode("some"), None, nodes.LiteralNode("BODY")]
            ),
            {},
            "someBODY",
            None,
        ...
        (
            nodes.ListNode([nodes.PlaceholderNode("a")]),
            {"a": "someBODY"},
            " someBODY",
            None,
        ),
...
def test_list_node_generate(
    node: nodes.ListNode, ctx: dict, expected_output: str, want_err: Optional[Exception]
) -> None:
    if want_err:
        with pytest.raises(want_err):
            node.generate(ctx=Context(ctx))
...
@pytest.mark.parametrize(
    ...
    [
        (
            nodes.RepeatNode(8, nodes.LiteralNode("a")),
            nodes.RepeatNode(8, nodes.LiteralNode("b")),
            False,
        ),
        (
            nodes.RepeatNode(8, nodes.LiteralNode("a")),
            nodes.RepeatNode(1, nodes.LiteralNode("a")),
            False,
        ),
        (
            nodes.RepeatNode(8, nodes.LiteralNode("a")),
...
def test_repeat_node_generate(
    node: nodes.RepeatNode, ctx: dict, expected_output: str
) -> None:
    assert expected_output == node.generate(ctx=Context(ctx))
